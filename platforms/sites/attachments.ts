...
export async function createAttachment(input: AttachmentInput,actor?:EquipmentActor) {
  ...
  const results = await db.batch([
    db.prepare(`INSERT INTO attachments (operation_id,equipment_id,object_key,filename,content_type,size,sha256,stage,caption,actor_user_id,actor_email,created_at)
      SELECT ?,?,?,?,?,?,?,?,?,?,?,? FROM equipment WHERE id=? ${actor?.role==="tecnico"?"AND assigned_member_id=?":""} ON CONFLICT(operation_id) DO NOTHING RETURNING ${selection}`)
      .bind(input.operationId,input.equipmentId,input.objectKey,input.filename,input.contentType,input.size,input.sha256,input.stage,input.caption,input.actorUserId,input.actorEmail,now,input.equipmentId,...(actor?.role==="tecnico"?[actor.memberId??-1]:[])),
