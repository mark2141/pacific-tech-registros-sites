...
export async function ensureStaff(email:string,role:Role){
  const db=getDb();const [existing]=await db.select().from(staff).where(eq(staff.email,email));if(existing)return existing as Staff;
  await db.insert(staff).values({email,name:email,role,protected:role==="admin"?1:0}).onConflictDoNothing();
  const [row]=await db.select().from(staff).where(eq(staff.email,email));return row as Staff;
}
export async function listStaff(before?:number){const rows=await getDb().select().from(staff).where(before?lt(staff.id,before):undefined).orderBy(desc(staff.id)).limit(51);return{users:rows.slice(0,50) as Staff[],nextCursor:rows.length>50?rows[49].id:null};}
export async function listTechnicians(){return getDb().select({id:staff.id,name:staff.name}).from(staff).where(and(eq(staff.role,"tecnico"),eq(staff.enabled,1))).orderBy(staff.name);}
export async function saveStaff(input:StaffInput,actor:{memberId:number;email:string},id?:number,version?:number){return getDb().transaction(async tx=>{
  ...
  await tx.insert(staffAudit).values({staffId:row.id,message:`${id?"Actualización":"Alta"}: ${input.email} · ${input.role} · ${input.enabled?"Activa":"Bloqueada"}`,actorEmail:actor.email});return row as Staff;
});}
