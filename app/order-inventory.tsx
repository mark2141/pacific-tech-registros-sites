...
export function OrderInventory({ financial, equipmentId, status, disabled, onBusy, onDirty, onChanged }: { financial:boolean;equipmentId: number; status: string; disabled: boolean; onBusy: (busy: boolean) => void; onDirty: (dirty: boolean) => void; onChanged: () => void }) {
  ...
  return <details className="order-inventory"><summary>Repuestos del inventario</summary><div className="order-inventory-body">
    ...
    {(!closed || source || dirty) && <form onSubmit={submit}><fieldset disabled={disabled || busy}>
      ...
      {item && <p className="field-hint">Disponibles: {item.stock}{financial&&<> · Costo compra: {formatMoney(source?.unitCostCents ?? item.unitCostCents)}</>}</p>}
