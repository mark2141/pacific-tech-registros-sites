...
export function EquipmentHistory({ equipmentId, version, legacyNote, note, onNoteChange, disabled, onBusyChange }: {
  ...
  return <section className="order-history" aria-labelledby="history-title">
    ...
    <ol className="history-list">{events.map(event => <li key={event.id}>
      <strong>{event.kind === "asignacion" ? "Asignación de técnico" : event.kind === "adjunto" ? "Archivo adjunto" : event.kind === "pago" ? "Movimiento de pago" : event.kind === "nota" ? "Nota" : event.kind === "repuesto" ? "Movimiento de repuesto" : event.kind === "contacto" ? "Contacto registrado" : event.kind === "ingreso" ? "Ingreso registrado" : `${labels[event.fromStatus || ""] || event.fromStatus} → ${labels[event.toStatus || ""] || event.toStatus}`}</strong>
