...
export async function accessJson(body:unknown,status=200){const user=await getAuthUser();return Response.json(operationalData(body,user?.role??"lectura"),{status,headers:{"Cache-Control":"private, no-store"}});}
